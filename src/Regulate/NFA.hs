...
instance IsString (MNFA s Char) where
  fromString = string

instance IsString (NFA Char) where
  fromString s = buildNFA (string s)
...
-- | Infix 'union'.
(>|<) :: Ord sigma => MNFA s sigma -> MNFA s sigma -> MNFA s sigma
(>|<) = union
...
optional :: Ord sigma => MNFA s sigma -> MNFA s sigma
optional = (epsilon `union`)

-- | Union of symbols.
oneof :: Ord sigma => [sigma] -> MNFA s sigma
...
-- | Modified @join@ that only produces each individual pair of strings once.
join' :: Ord sigma => NFA sigma -> NFA sigma -> NFA sigma
join' nfa1 nfa2 = runST $ do
  let g1 = graph nfa1
      g2 = graph nfa2
  ...
  -- map from (g1, g2) vertex pairs to g vertices.
  unstarredMapRef <- newSTRef Map.empty
  starredMapRef <- newSTRef Map.empty

  -- First, add all the vertices and build up the vertex map.
  forM_ (vertices g1) $ \v1 -> do
    forM_ (vertices g2) $ \v2 -> do
      v_unstar <- addVertex g
      v_star <- addVertex g
      -- Add new vertices to vertex maps
      modifySTRef unstarredMapRef (Map.insert (v1, v2) v_unstar)
      modifySTRef starredMapRef (Map.insert (v1, v2) v_star)
      -- If both vertices are the start state, then the new unstarred vertex is
      -- the joined start state
      when (v1 == s1 && v2 == s2) $
        writeSTRef startRef (Just v_unstar)
      -- If both vertices are final states, then both new vertices are final states
      when (v1 `Set.member` finals1 && v2 `Set.member` finals2) $
        modifySTRef finalsRef (Set.insert v_star . Set.insert v_unstar)

  unstarredMap <- readSTRef unstarredMapRef
  starredMap <- readSTRef starredMapRef
  ...
  forM_ (edges g1) $ \e -> do
    ...
    case a `Set.member` sharedSymbols of
      False -> forM_ (vertices g2) $ \v2 -> do
        let src = unstarredMap Map.! (edgeSource e, v2)
            dst = unstarredMap Map.! (edgeDest e, v2)
        void $ addLabeledEdge g src dst a
      True -> case Map.lookup a g2LeftEdgeMap of
        -- Look up the g2 edges with the same label, and add the corresponding
        -- vertices.
        Nothing -> return ()
        Just es -> forM_ es $ \e' -> do
          let src = unstarredMap Map.! (edgeSource e, edgeSource e')
              src_star = starredMap Map.! (edgeSource e, edgeSource e')
              dst = unstarredMap Map.! (edgeDest e, edgeDest e')
          void $ addLabeledEdge g src dst a
          void $ addLabeledEdge g src_star dst a

  forM_ (edges g2) $ \e -> do
    let c = fromJust (edgeLabel g2 e)
    case c `Set.member` sharedSymbols of
      True -> return ()
      False  -> forM_ (vertices g1) $ \v1 -> do
        let src = unstarredMap Map.! (v1, edgeSource e)
            src_star = starredMap Map.! (v1, edgeSource e)
            dst_star = starredMap Map.! (v1, edgeDest e)
        void $ addLabeledEdge g src dst_star c
        void $ addLabeledEdge g src_star dst_star c
  ...
  let start' = fromJust (Map.lookup (fromJust start) reduceMap)
      finals' = Set.fromList (catMaybes (flip Map.lookup reduceMap <$> Set.toList finals))
  return $ mkNFA g' start' finals'
  -- mkNFA <$> freeze g <*> pure (fromJust start) <*> pure finals
...
generateN' :: (Monoid m, Ord m)
           => (sigma -> m -> m)
           -> Int
           -- ^ maximum string length
           -> Vertex
           -- ^ Vertex to generate from
           -> NFA sigma
           -> [m]
           -- ^ Old strings + new strings
generateN' _ n v nfa | n <= 0 = if v `Set.member` finalStates nfa
                                then [mempty]
                                else []
generateN' f n v nfa =
  (if v `Set.member` finalStates nfa then [mempty] else []) ++
  [ f (fromJust (edgeLabel (graph nfa) e)) s
  | e <- outEdges (graph nfa) v
  , s <- generateN' f (n-1) (edgeDest e) nfa ]

-- | Run a breadth-first search for @n@ iterations on an NFA, returning the list
-- of strings produced.
generateN :: (Monoid m, Ord m) => (sigma -> m -> m) -> Int -> NFA sigma -> [m]
generateN f n nfa = generateN' f n (startState nfa) nfa

generateAll :: (Monoid m, Ord m) => (sigma -> m -> m) -> NFA sigma -> [m]
generateAll f nfa = go (startState nfa)
  where go v =
          (if v `Set.member` finalStates nfa then [mempty] else []) ++
          [ f (fromJust (edgeLabel (graph nfa) e)) s
          | e <- outEdges (graph nfa) v, s <- go (edgeDest e) ]
